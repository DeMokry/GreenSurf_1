...
public class Produkt_form extends JPanel implements ActionListener {
    ...
    JLabel lkategoria = new JLabel("kategoria");     //utworzenie etykiety pola do wprowadzania kategorii produktu
    JTextField kategoria = new JTextField(15);      // utworzenie pola wejsciowegodo wprowadzania kategorii produktu
    JLabel lnazwa = new JLabel("Nazwa");     //utworzenie etykiety pola do wprowadzania nazwy produktu
    JTextField nazwa = new JTextField(15);      // utworzenie pola wejsciowegodo wprowadzania nazwy produktu
    JLabel lcena = new JLabel("Cena");           //utworzenie etykiety pola do wprowadzania ceny produktu
    JTextField cena = new JTextField(15);       // utworzenie pola wejsciowegodo wprowadzania ceny produktu
    JLabel lpromocja = new JLabel("Promocja");   //utworzenie etykiety pola do wprowadzania promocji produktu
    JTextField promocja = new JTextField(15);   // utworzenie pola wejsciowegodo wprowadzania promocji produktu
    JLabel ldata = new JLabel("Data");           //utworzenie etykiety pola do wprowadzania daty produkcji produktu
    JTextField data = new JTextField(15);       // utworzenie pola wejsciowegodo wprowadzania daty produkcji produktu
    ...
    public void init() {
        ...
        add(cena);//dodanie pola do wprowadzania cenudo obiektu typu JPanel
        ...
        //obslugiwanyprzez obiekt typu Produkt_form za pomoca metody actionPerformed
    ...
    public String content_validate(JTextField val, int typ) { //walidacja danych
        ...
        if (s.equals("") || s.length() > 15) { //sprawdzenie, czy lancuchjest pusty lub dluzszyniż 15 znakow
            JOptionPane.showMessageDialog(this, "Lancuchdanych jest dluzszyniz 15 lub jest pusty");
            return null;
        } else {
            s = s.replaceAll(" ", "_"); //wyeliminowanie spacji z lancucha
            val.setText(s);
        }
        if (typ == 1) {// jeslisadane liczbowe, sprawdzenie, czy można dokonać parsowaniana liczbe
            try {
                Float.parseFloat(s);
    ...
    public String[] form_produkt() {
        if (content_validate(kategoria, 0) == null) //walidacja danych kategorii jako lancucha
        {
            return null;
        }
        ...
        String dane[] = {(String)kategoria.getText(), (String) nazwa.getText(), (String) cena.getText(), (String) promocja.getText()
        };//tablica z danymi produktu
        return dane;
    }
    ...
    public Date data() {
        ...
        try {
            String[] data2 = data1.split("-");//podziallancuchadaty np 12-12-2016 na tablicę lancuchow, reprezentujacychelementy daty
            rok = Integer.parseInt(data2[2]);
            miesiac = Integer.parseInt(data2[1]);
            dzien = Integer.parseInt(data2[0]);
        } catch (PatternSyntaxException | NumberFormatException | ArrayIndexOutOfBoundsException e) { //kontrola poprawności JOptionPane.showMessageDialog(this, "Bladformatu daty");//formatu daty
            return null;
        }
    ...
    public void actionPerformed(ActionEvent e) {
       ...
       GUI_main.getFacade().utworz_produkt(produkt);   //wywołanie metody
                                                       //logiki biznesowej tworzacej obiekt typu Produkt1
    }
}
